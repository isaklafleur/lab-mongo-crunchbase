      ...
      rl.question("Type an option: ", option => {
        switch (option) {
          ...
          case "11": // 11.- List the people that are working at Facebook right now (check relationships field)
            db
              .collection("companies")
              .find({ name: "Facebook" }, { "relationships.person": 1, _id: 0 })
              .toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question("\nType enter to continue: ", answer => {
                  ...
                  const names = result[0].relationships.map(
                    el => el.person.first_name + " " + el.person.last_name,
                  );
            ...
            db
              ...
              .toArray((error, result) => {
                if (error) {
                  ...
                  // console.log(JSON.stringify(result));
                  const isPastFalse = result[0].relationships.filter(
                    el => el.is_past === true,
                  );
            ...
            db
              ...
              .toArray((error, result) => {
                if (error) {
                  ...
                  const competitors = result[0].competitions.map(
                    el => el.competitor.name,
                  );
            ...
            db
              ...
              .toArray((error, result) => {
                if (error) {
                  ...
                  const companiesAndLocations = result.map(
                    el =>
                      "Company: " +
                      el.name +
                      " Address: " +
